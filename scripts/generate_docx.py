# ============================================================
# 交付文档生成脚本 — Generate Delivery DOCX
# ============================================================
# 功能：通过 python-docx 库生成一份专业格式的 Word 文档
# 输出：AIAgent全栈工程师-交付文档-终版.docx（根目录）
#
# 文档结构（14 章）：
#   封面 → 目录 → 项目概述 → 技术栈 → 系统架构 → 数据模型
#   → API 设计 → AI Agent 选型 → 部署协议 → 安全策略
#   → 失败恢复 → 可观测性 → 交付物清单 → 功能验证
#   → 完成度说明 → AI 协同记录
#
# 使用方式：
...
# ---- 全局样式常量 ----
FONT = "微软雅黑"                      # 统一中文字体
H1_COLOR = RGBColor(0x1E, 0x3A, 0x5F)  # 一级标题颜色（深蓝 #1E3A5F）
H2_COLOR = RGBColor(0x25, 0x63, 0xEB)  # 二级标题颜色（亮蓝 #2563EB）
HEADER_BG = "1E3A5F"                   # 表头背景色（深蓝）
ALT_ROW_BG = "F0F4FF"                  # 交替行背景色（淡蓝 #F0F4FF）
MUTED = RGBColor(0x88, 0x88, 0x88)     # 次级文字颜色（灰色 #888888）
...
def _rf(run, size=Pt(10), bold=False, color=None):
    ...
    Args:
        ...
        size: 字号（默认 10pt 正文字号）
        bold: 是否加粗
        color: 文字颜色（RGBColor 对象）

    关键点：
        run.font.name 设置西文字体
        run._element.rPr.rFonts.set(qn("w:eastAsia"), FONT) 设置东亚字体
        必须同时设置两个属性，否则中文回退到默认字体（宋体）
        qn("w:eastAsia") 将命名空间限定的 XML 属性名转换为实际 XML 属性
        "微软雅黑" 是无衬线字体，比宋体更现代专业
    """
    run.font.size = size
    run.font.name = FONT
    run._element.rPr.rFonts.set(qn("w:eastAsia"), FONT)  # 强制中文字体
    run.font.bold = bold
    if color:
...
def _shade(cell, color):
    ...
    python-docx 不直接支持单元格背景色，需通过底层 OXML 操作：
    1. 创建 w:shd（shading）元素
    2. 设置 w:fill 属性为颜色值（不带 # 号的 6 位 hex）
    3. 设置 w:val 为 "clear"（纯色填充模式）
    4. 将 shd 元素附加到单元格的 tcPr（table cell properties）
    """
    s = OxmlElement("w:shd")
...
def _cell(cell, text, size=Pt(8.5), bold=False, color=None, center=False):
    ...
    处理流程：
    1. 清空单元格默认文本（cell.text = "" 移除自动添加的空段落）
    2. 获取第一个段落对象
    3. 设置段落对齐方式（居中或左对齐）
    4. 添加文字 run 并应用字体样式
    """
...
def _table(doc, hdrs, rows, widths=None):
    ...
    参数:
        doc:     Document 对象
        hdrs:    表头列名列表（如 ["模块", "完成度", "说明"]）
        rows:    数据行列表（嵌套列表，每行对应表头列数）
        widths:  列宽列表（单位 cm），用于控制表格比例

    样式：
        - 表头：深蓝背景 + 白色文字 + 加粗 + 居中
        - 数据行：交替背景色（偶数行淡蓝），提高可读性
        - 全部单元格使用 "Table Grid" 样式（边框线）
    """
    ...
    for i, h in enumerate(hdrs):
        c = t.rows[0].cells[i]
        _cell(c, h, Pt(9), True, RGBColor(0xFF, 0xFF, 0xFF), True)
        _shade(c, HEADER_BG)
    ...
    for ri, row in enumerate(rows):
        for ci, val in enumerate(row):
            ...
            _cell(c, val, Pt(8.5))
            if ri % 2 == 1:
                _shade(c, ALT_ROW_BG)  # 偶数行（ri=1,3,5...）添加淡蓝背景
...
def _h(doc, text, level=1):
    ...
    处理流程：
    1. 使用 doc.add_heading 添加标准标题（自动应用 Heading N 样式）
    2. 遍历所有 runs 重新设置字体（覆盖 Word 默认字体）
    3. 根据 level 选择颜色：H1 深蓝，H2 亮蓝
    """
...
def build():
    ...
    设计原则：
    - 专业排版：统一的字体、字号、间距、配色
    - 内容丰富：表格展示技术细节，段落说明设计决策
    - 自解释：每个设计选择都附带"为什么"的说明
    """
    doc = Document()
    ...
    sty.font.size = Pt(9.5)           # 正文 9.5pt
    sty.font.name = FONT               # 默认字体
    sty.element.rPr.rFonts.set(qn("w:eastAsia"), FONT)
    sty.paragraph_format.space_after = Pt(6)    # 段后距 6pt
    sty.paragraph_format.line_spacing = 1.3     # 行距 1.3 倍

    # ── 封面 ──
    for _ in range(5):
        doc.add_paragraph("")          # 留白（5 行空段）
